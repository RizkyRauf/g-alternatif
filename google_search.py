...
Bisa dipanggil langsung: python google_search.py --query pertamina --pages 3 --category news --json
"""
...
VALID_CATEGORIES = [
    ...
    "files", "social_media", "it", "science", "map"
]
...
def build_params(query: str, page_no: int, category: str = DEFAULT_CATEGORY) -> dict:
...
def build_url(query: str, page_no: int, category: str = DEFAULT_CATEGORY) -> str:
...
def perform_search(query: str, max_pages: int, worker_id: int = 0, 
                   category: str = DEFAULT_CATEGORY) -> List[dict]:
    """
    ...
    Args:
        ...
        max_pages: Jumlah halaman yang akan discrape
        worker_id: ID worker untuk tracking (opsional)
        category: Kategori SearX (general, news, images, videos, dll)
    """
    if category not in VALID_CATEGORIES:
        print(f"Warning: Category '{category}' tidak dikenal. Menggunakan default '{DEFAULT_CATEGORY}'", 
              file=sys.stderr)
    ...
    with SB(
        ...
        for page_no in range(1, max_pages + 1):
            ...
            print(f"Page {page_no}/{max_pages} [{category}]: {url[:100]}...", file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SearX Scraper - Standalone dengan kategori fleksibel",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Contoh penggunaan:
  python google_search.py --query "berita terkini" --pages 3 --category news
  python google_search.py -q pertamina -p 2 -c images --json
  python google_search.py --query python --pages 5 --category general --output hasil.json

Kategori yang didukung:
  general, news, images, videos, music, files, social_media, it, science, map
        """
    )
    ...
    parser.add_argument("--pages", "-p", type=int, default=DEFAULT_PAGES, 
                       help="Jumlah halaman (1-20)")
    parser.add_argument("--category", "-c", type=str, default=DEFAULT_CATEGORY,
                       choices=VALID_CATEGORIES,
                       help=f"Kategori pencarian SearX (default: {DEFAULT_CATEGORY})")
    parser.add_argument("--worker-id", "-w", type=int, default=0, 
                       help="Worker ID untuk tracking")
    parser.add_argument("--json", "-j", action="store_true", 
                       help="Output JSON ke stdout")
    parser.add_argument("--output", "-o", help="Simpan hasil ke file JSON")

    args = parser.parse_args()

    # Validasi tambahan untuk pages
    if args.pages < 1 or args.pages > 20:
        print(f"Error: --pages harus antara 1-20", file=sys.stderr)
        sys.exit(1)

    print(f" Starting scraper: query='{args.query}', pages={args.pages}, "
          f"category='{args.category}', worker_id={args.worker_id}", file=sys.stderr)
    ...
    if args.json or args.output:
        ...
        if results:
            ...
            for i, item in enumerate(results[:3], 1):
                ...
                print(f"{i}. {title_preview} [{item['domain']}]", file=sys.stderr)
